...
class ObjectTypeEditorWidget(QWidget):
    ...
    def on_item_clicked(self, item):
        ...
        data = self.config.get_object_type(self.current_key)
        if data:
            self.name_edit.blockSignals(True)
            self.name_edit.setText(data.get("name", ""))
            self.name_edit.blockSignals(False)
            self.update_color_btn(self.color_btn, data.get("color", [150, 200, 255, 150]))
            self.update_color_btn(self.border_btn, data.get("border", [80, 130, 200]))
    ...
    def on_name_changed(self, text):
        if self.current_key:
            self.config.update_object_type(self.current_key, name=text)
    ...
    def pick_color(self):
        if not self.current_key:
            return
        data = self.config.get_object_type(self.current_key)
        curr = data.get("color", [150, 200, 255, 150])
        ...
        if color.isValid():
            ...
            self.config.update_object_type(self.current_key, color=rgba)
    ...
    def pick_border(self):
        if not self.current_key:
            return
        data = self.config.get_object_type(self.current_key)
        curr = data.get("border", [80, 130, 200])
        if len(curr) == 3:
            curr = curr + [255]
        color = QColorDialog.getColor(QColor(*curr), self, "Select Border Color")
        if color.isValid():
            rgb = [color.red(), color.green(), color.blue()]
            self.config.update_object_type(self.current_key, border=rgb)
            self.update_color_btn(self.border_btn, rgb)
            self.config_changed.emit()

    def add_type(self):
        import uuid
        key = f"obj_{uuid.uuid4().hex[:4]}"
        name = "New Object"
        color = [150, 200, 255, 150]
        border = [80, 130, 200]

        if self.config.add_object_type(key, name, color, border):
            self.load_types()
    ...
    def delete_type(self):
        ...
        if in_use:
            reply = QMessageBox.warning(
                self, "Type In Use",
                f"Type '{self.current_key}' is used by {len(in_use)} object(s).\n"
        ...
        self.config.delete_object_type(self.current_key)
        self.load_types()
        self.current_key = None
        self.form_widget.setEnabled(False)
        self.del_btn.setEnabled(False)
    ...
    def update_all(self):
        """Refresh all ObjectItems in scene."""
        if not self._scene:
            return
        from src.gui.items import ObjectItem
